...
case $option in
		...
		99) echo "#########################################"
			...
			echo "--------------------------------------------------------------------"
			echo "How to add tergets (ip-addresses of your exporters) for prometheus:"
			echo "sudo nano '/etc/prometheus/prometheus.yml'"
			echo "add ip-address and port of your server with node_exporter or pushgateway (in quotation marks separated by commas)."
			echo "systemcttl daemon-reload && systemctl restart prometheus"
			echo "you can check result here: http://<ip-address of prometheus>:9090/targets"
			echo "--------------------------------------------------------------------"
			echo "How to add source in Grafana:"
			echo "Go to http://<ip-address of grafana>:3000/datasources and click 'Add data source' button"
			echo "Choose 'Prometheus' and type ip-address of server with prometheus (localhost:9090). Click 'Save & Test' button"
			echo "--------------------------------------------------------------------"
